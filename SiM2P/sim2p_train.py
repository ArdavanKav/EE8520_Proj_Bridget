"""
Train SiM2P on paired 3D MRI-PET scans.
"""
...
from sim2p.resample import create_named_schedule_sampler
...
from sim2p.train_util import TrainLoop
...
def main(args):
    ...
    if dist.get_rank() == 0:
        ...
        # Use 'disabled' mode if debug is True (handles both bool True and string "True")
        wandb_mode = 'disabled' if (args.debug is True or str(args.debug).lower() == 'true') else 'online'
        wandb.init(project="sim2p", group=args.exp, name=name, config=vars(args), mode=wandb_mode)
        logger.log("creating model and diffusion...")
    
    # save args into txt file
    arg_file = os.path.join(workdir, 'args.txt')
    ...
    TrainLoop(
        ...
        ssp_projector_path=args.ssp_projector_path,
        **sample_defaults()
    ).run_loop()
