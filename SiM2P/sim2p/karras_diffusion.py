...
def vp_logsnr(t, beta_d, beta_min):
    t = th.as_tensor(t)
    # Clamp t to avoid log(0) when t=0, which causes infinity
    t = th.clamp(t, min=1e-5)
...
class KarrasDenoiser:
    ...
    def training_bridge_losses(self, model, x_start, sigmas, ssp_projector=None, model_kwargs=None, noise=None):
        ...
        diff_output = denoised

        weights = self.get_weightings(sigmas)
        
        weights =  append_dims((weights), dims)
        terms["xs_mse"] = mean_flat((diff_output - x_start) ** 2)
        
        LOSS_TYPE = 'mse'
        if LOSS_TYPE == 'mae':
            terms["mae"] = mean_flat(weights * abs(diff_output - x_start))
            ...
            terms["mse"] = mean_flat(weights * (diff_output - x_start) ** 2)
        ...
        if ssp_projector is not None:
            ssp_projector.to(diff_output.device)
            ssp_scan = model_kwargs.get('ssp_scan', None)
            assert ssp_scan is not None, "Please provide ssp_scan if using the ssp projector"
            ssp_recon = ssp_projector(diff_output)
            ssp_loss = mean_flat(weights * (ssp_recon - ssp_scan) ** 2) # mse loss

            terms["ssp_loss"] = ssp_loss
            terms["loss"] = terms["loss"] + 0.01 * terms["ssp_loss"]
    ...
    def denoise(self, model, x_t, sigmas ,**model_kwargs):
        ...
        model_output = model(c_in * x_t, rescaled_t, **model_kwargs)  # F_\theta(c_in * x_t, t)
        denoised = c_out * model_output + c_skip * x_t   # D_\theta(x_t, t) = c_out * F_\theta(c_in * x_t, t) + c_skip * x_t
