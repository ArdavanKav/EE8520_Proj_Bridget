...
class TrainLoop:
    def __init__(
        ...
        **sample_kwargs,
        ...
        # Create a dedicated debug log file for step-by-step tracking
        self.debug_log_path = self.workdir + '/step_losses.log'
        if dist.get_rank() == 0:
            with open(self.debug_log_path, 'w') as f:
                f.write("step,rank,loss,mse,xs_mse,t_min,t_max,has_nan,has_inf\n")
        ...
        # Use DDP if we have multiple ranks (world_size > 1)
        if th.cuda.is_available() and dist.get_world_size() > 1:
            self.use_ddp = True
            local_rank = dist_util.get_local_rank()
            print(f"[Rank {dist.get_rank()}] Creating DDP model on local device {local_rank}...", flush=True)
            sys.stdout.flush()
            self.ddp_model = DDP(
                self.model,
            ...
            print(f"[Rank {dist.get_rank()}] DDP model created!", flush=True)
            sys.stdout.flush()
        else:
    ...
    def _load_and_sync_parameters(self):
        ...
        # Always sync parameters from rank 0 to ensure all ranks have identical weights
        # This is critical for DDP initialization
        if dist.get_world_size() > 1:
            print(f"[Rank {dist.get_rank()}] Syncing model parameters from rank 0...", flush=True)
            dist_util.sync_params(self.model.parameters())
            print(f"[Rank {dist.get_rank()}] Model parameters synced!", flush=True)
    ...
    def run_loop(self):
        ...
        steps_per_epoch = len(self.data.dataset) // self.global_batch
        if steps_per_epoch == 0:
            steps_per_epoch = 1
        
        # Track current epoch based on step count
        current_epoch = self.resume_step // steps_per_epoch + 1
        best_val_loss = 999999.0
        
        print(f"[Rank {dist.get_rank()}] Entering run_loop, steps_per_epoch={steps_per_epoch}", flush=True)
        sys.stdout.flush()
        
        if dist.get_rank() == 0:
            logger.log(f"=== Starting Training (steps per epoch: ~{steps_per_epoch}) ===")
        
        # The data loader is infinite, so we iterate through it continuously
        for batch_data in self.data:
            # Calculate current epoch from step count
            new_epoch = self.step // steps_per_epoch + 1
            if new_epoch > current_epoch:
                current_epoch = new_epoch
                if dist.get_rank() == 0:
                    logger.log(f"=== Starting Epoch {current_epoch} ===")
            ...
            if took_step and self.step % self.log_interval == 0:
                ...
                for key, value in log_items:
                    ...
                    try:
                        float_val = float(value) if not isinstance(value, str) or not '%' in value else None
                        if float_val is not None:
                            self.writer.add_scalar(f'logs/{key}', float_val, global_step=logs['step'])
                ...
                if dist.get_rank() == 0:
                    ...
                    xs_mse_str = f"xs_mse: {logs.get('xs_mse', 'N/A')}"
                    print(f"Step {self.step} | Epoch {logs.get('epoch', '?')} | {xs_mse_str} | {loss_str} | lg_loss_scale: {logs.get('lg_loss_scale', 'N/A')}")
    ...
    def forward_backward(self, batch, cond, train=True):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # Detailed loss logging every 10 steps
            if self.step % 20 == 0:
                import torch
                loss_val = loss.item()
                mse_val = losses.get("mse", losses.get("loss")).mean().item()
                xs_mse_val = losses.get("xs_mse", torch.tensor(0.0)).mean().item()
                ...
                has_nan = torch.isnan(loss).any().item() or torch.isnan(losses["loss"]).any().item()
                has_inf = torch.isinf(loss).any().item() or torch.isinf(losses["loss"]).any().item()
                
                log_msg = (f"[Step {self.step}] [Rank {dist.get_rank()}] loss={loss_val:.6f} mse={mse_val:.6f} xs_mse={xs_mse_val:.6f} "
                          f"t=[{t_min:.4f},{t_max:.4f}] weights=[{weights_min:.4f},{weights_max:.4f}] "
                          f"NaN={has_nan} Inf={has_inf}")
                print(log_msg, flush=True)
                sys.stdout.flush()
                
                # Also write to dedicated log file for reliable tracking
                try:
                    with open(self.debug_log_path, 'a') as f:
                        f.write(f"{self.step},{dist.get_rank()},{loss_val:.6f},{mse_val:.6f},{xs_mse_val:.6f},{t_min:.4f},{t_max:.4f},{has_nan},{has_inf}\n")
                        f.flush()
                except:
                    pass
    ...
    def log_step(self):
        ...
        if steps_per_epoch == 0:
            steps_per_epoch = 1
        epoch = self.step // steps_per_epoch + 1
        epoch_progress = (self.step % steps_per_epoch) / steps_per_epoch * 100
        
        logger.logkv("step", self.step)
        logger.logkv("epoch", epoch)
        logger.logkv("epoch_progress", f"{epoch_progress:.1f}%")
        logger.logkv("samples", (self.step + 1) * self.global_batch)

    def save(self, for_preemption=False):
        if dist.get_rank() == 0:
            logger.log(f">>> Saving checkpoint at step {self.step} <<<")
