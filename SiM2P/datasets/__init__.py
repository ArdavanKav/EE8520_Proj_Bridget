...
def load_data(
    ...
    if not test_only:
        if dataset == 'mri2pet':
            from .mri2pet import Mri2petDataset
            trainset = Mri2petDataset(img_size=image_size, data_path=[data_dir], stage='train', target_modality=target_modality)
            valset = Mri2petDataset(img_size=image_size, data_path=[data_dir], stage='val', target_modality=target_modality)
            if include_test:
                testset = Mri2petDataset(img_size=image_size, data_path=[data_dir], stage='test', target_modality=target_modality,)
        else:
        ...
        if dataset == 'mri2pet':
            from .mri2pet import Mri2petDataset
            testset = Mri2petDataset(img_size=image_size, stage='test', target_modality=target_modality,)
