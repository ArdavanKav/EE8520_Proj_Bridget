...
class Mri2petDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Only resize if data is not already the target size
        if mri_scan.shape[0] != self.img_size:
            ...
            # Data is already correct size - just convert to tensor with channel dim
            mri_scan = torch.from_numpy(mri_scan[np.newaxis].copy()).float()
            pet_scan = torch.from_numpy(pet_scan[np.newaxis].copy()).float()
